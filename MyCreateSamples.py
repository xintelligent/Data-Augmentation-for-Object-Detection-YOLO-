...
def main():
    ...
    backgroundFilePath = parser.get('MY_PARAMS', 'backgroundFilePath')
    samplePath = parser.get('MY_PARAMS', 'sampleFilesPath')
    outputfolder = parser.get('MY_PARAMS', 'OutputPath')
    ...
    bkgFileLoader.loadbkgJPEGFiles(backgroundFilePath)
    t = time.time()
    start = int(round(t * 1000))  # 毫秒级时间戳
    image_count = 0
        ...
        while count < outputPerSample:
            ...
            if flag:
                ...
                with open(os.path.join(outputfolder, str(outputName + '.txt')), 'w') as f:
                    details = '1 ' + ' '.join(str(coord) for coord in np.reshape(finalBoundRect, 4)) + '\n'
                    f.write(details)
                count = count+1
            else:
                outputName = filename + '_' + str(count)
